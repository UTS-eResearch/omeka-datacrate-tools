...
with args["infile"] as cat:
    dc = json.load(cat)


key_identity = args["identity"] #"GMauLUNqOe8ifubbeapIwPDV9k7uRfo5"
key_credential = args["credential"] # "ReRyOeh5HcvLXy6UjPfZziHBI9tFmSM3"
...
with open("context.json") as cat_file:
    context = json.load(cat_file)
                    ...
                    for f in item['hasFile']:
                        ...
                        if "original_" in  f["@id"]:
                            ...
                            if upload_this_file:
                                ...
                                files = [("file[%s]" % str(index), open(f["@id"],'rb').read())]
                                #s = requests.Session()
                                ...
                                curl_command = ["/usr/bin/curl", "-F", "'file[%s]=@%s'" % (index, f['@id']), "-F", "data='%s'" % json.dumps(data), "'%smedia?%s'" % (host_url, auth), "2> ", "/dev/null"]
